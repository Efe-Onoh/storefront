  ...
  const SpecialOffers = () => (
    <Grid item>
      <Grid
        ...
        style={{ background: "linear-gradient(180deg, #E0A30B 0%, #FFCF57 100%)" }}
        direction={"column"}
        rowGap={2}
        alignItems={"center"}
      ...
      >
        <Grid item>
          <Grid container direction={"column"} textAlign={"center"} rowGap={2}>
            <Grid item>
              <Typography variant="ManropeBold18">Special offers</Typography>
            </Grid>
            <Grid item>
              <Button className="btn contained sidebarad" variant="contained">
                View All
              </Button>
            </Grid>
          </Grid>
    ...
    <Grid item>
      <Grid container className="sidebar-ad" style={{ background: "linear-gradient(180deg, #E17838 0%, #EB8F58 100%)" }} direction={"column"} rowGap={2} alignItems={"center"}>
        <Grid item>
          <Grid container direction={"column"} textAlign={"center"} rowGap={2}>
            <Grid item>
              <Typography variant="ManropeBold18">
                Product
                <br />
                Recommendations
              </Typography>
  ...
  return (
    <Grid className="landingPageBody">
      ...
      {AdsArray.map((Ad, index) => (
        <ProductList key={index} SideBarAd={Ad} />
      ))}
      <BlogPosts />
      <Footer />
    </Grid>
